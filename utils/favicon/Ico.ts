...
export default class Ico {
  ...
  constructor(canvas: HTMLCanvasElement, sizes: number[] = [16, 32, 48]) {
    this.canvas = canvas;
    this.sizes = sizes;
  }
  ...
  createBitmapImageData(canvas: HTMLCanvasElement): string {
  ...
  canvasToBitmap(canvas: HTMLCanvasElement): ArrayBuffer {
    const ctx = canvas.getContext("2d")!;
    const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
  ...
  Uint8ArrayToBinary(Uint8Array: Uint8Array): string {
    let binary = "";
